...
#define APPLETB_BASIC_CONFIG	1	/* HID-only "basic" config (this driver) */
#define APPLETB_OSX_CONFIG	2	/* "OS X" config: display + plain HID */

static int appleib_config = APPLETB_BASIC_CONFIG;
module_param_named(config, appleib_config, int, 0444);
MODULE_PARM_DESC(config,
		 "iBridge USB configuration to select:\n"
		 "    [1] - basic: drive the touch bar/ALS as HID via this driver\n"
		 "    2   - OS X: expose the touch bar display (appletbdrm) and the\n"
		 "          touch input as plain HID; this driver only selects the\n"
		 "          configuration and otherwise steps aside");
			     ...
			     const struct hid_device_id *id)
{
	...
	if (udev->actconfig->desc.bConfigurationValue != appleib_config) {
		rc = usb_driver_set_configuration(udev, appleib_config);
		return rc ? rc : -ENODEV;
	}

	/*
	 * In the OS X configuration the touch bar is driven as a USB display
	 * (appletbdrm) plus a plain HID touch device handled by the generic HID
	 * stack, so we only select the configuration and leave these interfaces
	 * to those drivers.
	 */
	if (appleib_config != APPLETB_BASIC_CONFIG)
		return -ENODEV;
